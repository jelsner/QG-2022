...
A former PhD student from this department is looking for a masters-level geographer to assist with disease mapping and basic spatial analysis as applied to livestock disease. The job entails supporting scientists (virologists, field epidemiologists) and post-docs (some with spatial analysis expertise) in mapping and visualizing disease occurrence. Encouraged to develop independent projects leading to publication. Flexible work location especially if working toward a PhD.
...
Spatial data analysis often requires calculations on the geometry. Two of the most common are computing centroids (geographic centers) and computing buffers.

Geo-computation should be done on coordinates that have been projected. To see what CRS is currently assigned to the simple feature data frame you use the `sf::st_crs()` function.

```{r}
sf::st_crs(USA_48.sf)
```
The output is in well-known text format and includes the datum, shape of the Earth, and coordinates (geodetic).
...
You calculate the geographic centroid for each of the lower 48 states with the `sf::st_centroid()` function.
...
The land area (`aland`) makes sense when the geometry is `MULTIPOLYGON` it does not make when the geometry is `POINT`.

You map the points using the `tmap::tm_dots()` function after first mapping the state borders.
...
You can overlay data to the raster map. Let's consider a map of Chicago.
...
Chicago has a data portal publishing a large volume of public records. Here you look at crime data from 2017. The file `car_thefts.csv` is a spreadsheet obtained from that portal with a list of car thefts.

You read these data using the `readr::read_csv()` function.

```{r}
carTheft.df <- readr::read_csv(file = here::here("data", "car_thefts.csv"))
head(carTheft.df)
```
...
Instead of relying on `geom_point()` and plotting the raw data, another approach is to create a heat map. This is done with a density estimator. Since the map has two dimensions is called a 2-D kernel density estimation (KDE).
...
```{r}
ggmap::ggmap(CHI_map) +
  geom_density_2d(data = carTheft.df,
                  mapping = aes(x = Longitude,
                                y = Latitude))
```
...
```{r}
ggmap::ggmap(CHI_map) +
  stat_density_2d(data = carTheft.df,
                  mapping = aes(x = Longitude,
                                y = Latitude,
                                fill = stat(level)),
                  geom = "polygon")
...
Download the `.Rmd` file from Canvas.
...
a.  Assign this data frame to a new data frame object called `TCs.df` (for tropical cyclones). Then print the column names and dimensions of this data frame. (5)

b.  Use functions from the {ggplot2} package to create a histogram of the wind speed (`wind`). Use 11 bins. Wind speeds are in mph. Label horizontal axis accordingly (5)

c.  Create a scatter plot showing the relationship between wind speed (`wind`) and minimum sea-level air pressure (`pressure`) in hPa (hecto-pascals). Put the pressure variable on the horizontal axis. Use the `geom_smooth()` function to add a local regression smooth line through the set of points. Label the axes accordingly. (10)
