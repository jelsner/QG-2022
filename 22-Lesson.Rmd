...
Previously focus was on computing spatial autocorrelation in a variable aggregated at some areal-unit level. You saw examples Moran's I computed on crime rates and police expenditures aggregated at the county level. This is important for hot-spot analysis or spatial sampling.

However, if you want a model for a response variable as a function of explanatory variables across a set of areal units then focus needs to shift to computing spatial autocorrelation in model residuals.

The model of choice is a linear regression, but when the residuals from a linear regression exhibit significant spatial autocorrelation then you should consider a _spatial_ regression model.
...
model.lm <- lm(f, data = CC.sf)

model.lm |>
  summary()
```

The output shows that the regression model statistically explains 55% of the variation in crime (multiple R-squared). 
...
Justification for the spatial lag model comes from domain specific knowledge but is motivated by a 'diffusion' process. The response variable $y_i$ is influenced by the explanatory variables at location $i$ and by explanatory variables at locations $j$.
...
The total effect (TE) is the sum of the direct and indirect effects. It measures the cumulative impact on crime arising from one tract $j$ increasing its income over all other tracts (on average). It is given by 
...
If there are large differences (e.g., different signs) between the coefficient estimate from SEM and the linear model, this suggests that neither model is yielding parameters estimates matching the underlying the data generating process.
