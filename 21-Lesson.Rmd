...
## Other neighborhood definitions {-}

Autocorrelation plays a central role in spatial statistics by quantifying the degree to which nearby things tend to cluster. Moran's I is one metric of spatial clustering. 

The value of Moran's I depends on how 'nearby' is defined (neighborhood definition) and on how the neighbors are weighted. Various neighbor definitions are possible and the choice will influence the value of any statistic of spatial autocorrelation.
...
Variables in the simple feature data frame include police expenditures (`POLICE`), crime (`CRIME`), income (`INC`), unemployment (`UNEMP`), percentage of people identifying as white (`WHITE`) and other socio-economic characteristics across Mississippi at the county level. 

Police expenditures are per person (1982 dollars). Crime is the number of serious crimes per 100,000 people (1981). Personal income is per person (1982 dollars). Unemployment is percent of people looking for work (1980).
...
To estimate autocorrelation for any variable in the data frame, you need to first define neighbors and assign weights for each polygon.

The default options in the `spdep::poly2nb()` and `spdep::nb2listw()` result in neighbors defined by 'queen' contiguity (polygon intersections can include a single point) and weights assigned using row standardization (the sum of the weights equals the number of regions).
...
Alternatively you can specify the number of neighbors and then define neighbors based on proximity (closeness). Here you first extract the coordinates of the polygon centroids as a matrix.
...
Then use the `spdep::knearneigh()` function (K nearest neighbors) on the coordinate matrix and specify the number of neighbors with the `k =` argument. Here you set it to six. That is, allow each county to have 6 neighbors based on proximity.
...
A look at the first six rows of the matrix.
...
The first county has county 9, 3, 2, 4, 10, and 14 as neighbors.

Note that by using distance to define neighbors the matrix is not symmetric. For example, county 3 is a neighbor of county 2 (row 2 contains a 3), but county 2 is not a neighbor of county 3 (row 3 does not contain a 2).

Certain statistical models require the neighbor matrix to be symmetric. That is if region X is a neighbor of region Y then region Y must be a neighbor of region X.
...
A difference between the two neighborhoods is the number of links on counties along the state borders. The nearest-neighbor defined neighborhoods have more links. Note: when neighbors are defined by proximity counties can share a border but they still may not be neighbors.

Choice of neighbors is based on domain specific knowledge. If the process you are interested in can be described by a dispersal mechanism then proximity definition might be the right choice for defining neighbors. If the process can be described by a border diffusion mechanism then contiguity might be the right choice.
...
Values of Moran's I are constrained between -1 and +1. In this case the neighborhood definition has little impact on inferences made about spatial autocorrelation. The kurtosis is between 2 and 4 consistent with a set of values from a normal distribution.
...
Statistical tests provide a way to guard against being fooled by this chance autocorrelation. So it is important to ask, is the value of Moran's I significant with respect to the null hypothesis of no autocorrelation?
...
Compute a spatially-lagged unemployment variable. Start by putting the unemployment values in a vector called `unemp` then use the `spdep::lag.listw()` function with the standardize weights as the first argument and the vector of the unemployment as the second argument.
...
Under the assumption of normally distributed and uncorrelated data, the expected value for Moran's I is -1/(n-1) where n is the number of regions. 
...
Use `dplyr::sample_n()` to generate one shuffle and make a map.
...
The red line indicates the value of Moran's I using the data in the correct counties. All but nine of values are lower (to the left of the red line).
...
The knowledge is more useful after other factors are considered. Knowledge of significant autocorrelation in the model residuals can help you improve the model.
...
The Moran's I statistic was first used in the 1950s. Localization of the statistic was presented by Luc Anselin in 1995.
...
```{r}
Ii_stats <- PE.sf$WHITE |>
  spdep::localmoran(listw = wts)
...
Areas where percent white is high over the northeast are also areas with the largest spatial correlation. Other areas of high spatial correlation include the Mississippi Valley and in the south. The county with the most negative spatial correlation (green) is the county in the northwest with a fairly high percentage of whites neighbored by counties with much lower percentages of whites.