...
-   Graphing examples
-   Introduction to working with base R
-   Data as vectors
...
Using the {ggplot2} functions, you answer the question above by mapping the `Temp` variable to the x aesthetic and mapping the `Ozone` variable to the y aesthetic.

More simply you could say that you plot `Temp` on the x axis and `Ozone` on the y axis. Put you want to acknowledge that the axes are aesthetics (there are others like color, size, etc).
...
To suppress the warning add the argument `na.rm = TRUE` in the `geom_point()` function.
...
To help better understand the relationship you add another layer. This layer is defined by `geom_smooth()` which takes the same aesthetics.
...
In the above code you use the same mapping (`x = Temp` and `y = Ozone`) in the point layer (`geom_point()`) and in the smooth layer (`geom_smooth()`). 

By putting the mapping argument in the `ggplot()` function, you do not need to re-specify the mapping in the geom layers.
...
Since you have the template from the previous code. You first copy/paste and then simply change the x-axis aesthetic to `Wind` and the y-axis aesthetic to `Ozone`.
...
The result is a scatter plot that has the values of ozone as the location in the 2-D plot instead of points.

To map an aesthetic to a variable, provide the name of the aesthetic to the name of the variable inside the `aes()` function.
...
Example, consider the Palmer penguin data set from <https://education.rstudio.com/blog/2020/07/palmerpenguins-cran/>.
...
Note that this object is now located in our environment. It is a character string involving letters, backslashes, etc and it is given inside double quotes.

Next you get the data and save it as an object called `penguins.df` with the `read_csv()` function from the {readr} package. Inside the parentheses of the function you put the name of the location.

```{r}
penguins.df <- readr::read_csv(loc)
```

Note that the object `penguins.df` is now in your environment. It is a data frame containing 344 rows (observations) and 8 variables. You list the first 10 rows and 7 columns by typing the name of the object as follows.
...
Each penguin belongs to one of three species. To see how many of the 344 penguins are in each species you use the `table()` function. Between the parentheses of this function you put the name of the data `penguins.df` followed by the `$` sign followed by the name of the column `species`.

```{r}
table(penguins.df$species)
```

Said another way, you reference columns in the data with the `$` sign so that `penguins.df$species` is how you refer to the column `species` in the data object named `penguins.df`.
...
The colors reveal that the unusual points (on the right side of the plot) are two-seaters. Sports cars have large engines like SUVs and pickup trucks, but small bodies like midsize and compact cars, which improves their gas mileage. In hindsight, these cars are unlikely to be hybrids since they have large engines.
...
Syntax is important. You get an error message when you type a function that R does not understand. For example:
...
Data vectors have a type. One restriction on data vectors is that all the values have the same type. This can be numeric, as in counts, character strings, as in
...
Work smarter. R's console keeps a history of your commands. The previous commands are available using the up and down arrow keys. Repeatedly pushing the up arrow will scroll you backward through the history file, so you can reuse previous commands.

Many times you wish to change only a small part of a previous command, such as when a typo is made. With the arrow keys you access the previous command then edit it.