...
All the AIC values exceed those in the first row so we are content with this model.

Next we map the predicted values. We first add the predicted values to the simple features data frame as a column with name `predLM2`. The predicted values from the model object are extracted with the `predict()` method.
```{r}
...
Since the homicide rates are non-negative, we create a new column in the `SH.sf` data frame called `logHR90`, which is the logarithm of `HR90`.

Since there are some counties with no homicides [log(0) = -Inf] we change values in those counties to the minimum observed value before taking logarithms. 

Here we first create a logical vector `x` corresponding to the rows with non-zero homicide rates. We then find the minimum non-zero rate and assign it to `e`. Next we subset on this value for all rates equal to zero and finally we create a new column as the logarithm of the non-zero rates.
```{r}
x <- SH.sf$HR90 != 0
e <- min(SH.sf$HR90[x])
SH.sf$HR90[!x] <- e
...
lm.morantest(model.lm3, 
             listw = wts)
```

Moran I is only .11 but it is statistically significant ($p$ value < .01) because there are many counties.

Next we map the residuals. First we add the residuals as a column named `res3` in the simple feature data frame.
```{r}
...
We do this with functions from the {spgwr} package. The geometry information in simple feature data frames is NOT accessible by functions in this package so we need to create another type of spatial data frame.
```{r}
...
We obtain the optimal bandwidth with the `gwr.sel()` function from the {spgwr} package. We include the model formula, the data, and since the CRS is geographic we use the argument `longlat = TRUE` to get the distances in kilometers.
```{r}
library(spgwr)
...
To get a feel for what a bandwidth distance of 165 km means in terms of the average number of neighbors per county we note that one-half the distance squared times $\pi$ is the area captured by the bandwidth.
```{r}
( bwA <- pi * (bw * 1000 /2)^2 ) 
```
...
We then use the `gwr()` function from the {spgwr} package to includes the formula, data, and the `bandwith =` argument.
```{r}
...
Key point: When we fit a regression model to data that vary spatially we are assuming an underlying stationary process. This means we believe the explanatory variables 'provoke' the same statistical response across the entire domain. 

If this is not the case then it shows up in a map of correlated residuals. One approach to investigate things further is to use geographic regression. Another approach is to use a single spatial regression model.

## Assignment 11

Due Date: December 4, 2020 (3pm)

Total Points: 50

**1** Consider the demographic data at the county level in the state of Georgia located in the folder _Georgia_ as shape files.

(a) Import the data to create a simple feature data frame called `GE.sf`. Remove the row with `FID_1` equal to 65. Hint: Use the `read_sf()` function from {sf} and then the `filter()` function from {dplyr} with `FID_1 != 65`. (5)
(b) Plot a map showing the county borders. Hint: Apply the `plot()` method to the `geometry` column of the simple feature data frame (3)
(c) Create a spatial weights matrix using queen contiguity. Hint: This is the default when using the `poly2nb()` function from {spedep}. (5)
(d) Compute the Moran I for the variable percent elderly (`PctEld`) and test for statistical significance. Hint: Use the `moran.test()` function. (3)
(e) Compute local Moran I for the variable percent elderly. Hint: Use the `localmoran()` function. (4)
(f) Create side-by-side choropleth maps showing county-level percent elderly values and values of local Moran I. Hint: Use the `tm_fill()` function from the {tmap} package. (5)

**2** Consider the police expenditure data at the county level in the state of Mississippi located in the folder _police_.

(a) Import the data to create a simple feature data frame called `PE.sf`. (3)
(b) Create a new variable in the data frame that is the logarithm of the police expenditure. Name the column `logPOLICE`. (3)
(c) Create a spatial polygons data frame from the simple feature data frame using the `as()` function. Call this data frame `PE.sp`. (3)
(d) Use the `gwr.sel()` function from the {spgwr} package to determine the bandwidth for a geographic regression of `logPOLICE` onto `CRIME` and `INC`. Save the bandwidth in an object called `bw`. (3)
(e) Fit the geographic regression model of `logPOLICE` onto `CRIME` and `INC` using the `gwr()` function. Assign the model output to the object `model.gwr`. (5)
(f) Add the local R squared output from the model to the simple feature data frame. Hint: This output is in the spatial data frame `model.gwr$SDF`. (5)
(g) Create a choropleth map of the local R squared values using functions from the {tmap} OR {ggplot2} packages. (3)