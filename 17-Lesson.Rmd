...
Suppose we are interested in predicting maximum heart rate by age using 15 randomly chosen people who are tested for maximum heart rate. The data are listed below. Age is in years and heart rate is in beats per minute. Create a data frame called `heart.df`.
```{r}
...
We start with a scatter plot of the data.
```{r}
library(ggplot2)
...
Heart rate appears to decrease linearly with increasing age. 

Next we fit a linear regression model so that we can predict heart rates from age.
```{r}
model <- lm(HR ~ Age, 
            data = heart.df)
model
...
We interpret the model as follows: Given the sample of data and the linear regression model, we find that on average a person's maximum HR _decreases_ by .8 bpm every year or 8 bpm every decade.
...
From the table of coefficients we see that the best estimate for the slope is -.798 with a margin of error of +/- .07. The slope estimate is for a population of all people. The $t$ value for the zero-slope hypothesis is computed as the slope value divided by its standard error.

Here the $t$ value is -11.4 which gives a $p$-value (Pr(>|t|)) less than .001. So we have overwhelming evidence that, given these data, on average maximum heart rate depends on age (statistically).
...
SSY is the deviance output from a constant mean model. The constant mean model is estimated by setting the explanatory variable to a constant `1`. We call this `model0`.
```{r}
model0 <- lm(HR ~ 1, 
             data = heart.df)
SSY <- deviance(model0)
...
Although we don't have access to a new set of subjects, we can create samples from the original set of subjects (bootstrapping). We start by creating a sample of size 15 from the integers 1 through 15 at random with replacement using the `sample()` function.
```{r}
...
To get a bootstrap sample of our heart rate and corresponding age variables we use these sampled integers as indexes in the vectors of `HR` and `Age`.
```{r}
bsHR <- heart.df$HR[I1]
...
We then fit the regression model using the bootstrap sample and call this model `modelBS1`.
```{r}
modelBS1 <- lm(bsHR ~ bsAge)
```

Note that we don't need the `data = ` argument since `bsHR` and `bsAge` are vectors with corresponding elements.

We are interested only in the slope coefficient from the regression model. This is printed using the `coef()` on the model object. 
```{r}
coef(modelBS1)
```

The output is a vector of length two with the first element the intercept coefficient and the second element the slope coefficient. To print only the slope coefficient we use the bracket notation. We print the slope coefficient from the model fit using the actual data and the slope coefficient from the model fit using the bootstrap sample data.
```{r}
coef(model)[2]
coef(modelBS1)[2]
```

The slope values are somewhat different as expected. We can repeat the sampling procedure many times to get many slope values.

For example, in the code below we repeat this procedure (randomly selecting integers and corresponding values of the variables) 1000 times saving the slope and intercept coefficients as vectors. We then create a histogram of the slope values adding a vertical red line corresponding the actual slope.
```{r}
slope <- numeric()
int <- numeric()
for(i in 1:1000){
...
The `predict()` function (method) is used to make a prediction with our model. We specify the values of the explanatory variable as a data frame.
```{r}
predict(model, 
        newdata = data.frame(Age = c(50, 60)))
```

To get uncertainty intervals on the predicted values we include the `level =` and `interval = "confidence"` arguments.
```{r}
...
We state that the best prediction for average maximum heart rate for a set of random 50-yr old people is 170 bpm with a 95% uncertainty interval between 167 and 173 bpm. Thus if we repeat the sampling 100 times and make the same prediction, our CI on the prediction will cover the true predicted value 95 times.

These so called "point wise" uncertainty intervals are used to construct the gray ribbon when we use the `geom_smooth()` layer. To see this we plot the point wise interval for heart rates for 50-yr olds using the `geom_segment()` layer.
```{r}
ggplot(data = heart.df, 
       mapping = aes(x = Age, y = HR)) +
  geom_point() +
...
By default `se = TRUE` in the `geom_smooth()` function so the ribbon is plotted. The default uncertainty width is specified with `level = .95`. If we want more certainty we increase the level toward 1. This will make the ribbon wider.
...
With `interval = "prediction"` in the `predict()` method we get a 95% 'prediction' interval. Here we estimate the prediction interval for a 60-yr old patient.
```{r}
predict(model, data.frame(Age = 60), 
        level = .95, interval = "prediction")
```

That interval is wider than the 'confidence' interval as it represents both sources of uncertainty.

To plot the prediction band, first add the prediction intervals to the original data frame, assigning the result to a new data frame. Then order the observations in this new data frame by the rank of the explanatory variable.
```{r}
...
## Example: Predicting flipper length from body mass
...
The data set `homedata` {UsingR} contains assessed housing values in Maplewood, NH for the years 1970 (`y1970`) and 2000 (`y2000`). Suppose you're interested in predicting house values in the year 2000 from house values in the year 1970 using a linear regression model.

* Graph the data as a scatter plot and include the least-squares (best fit) regression line
```{r}
library(UsingR)

ggplot(data = homedata, 
       mapping = aes(x = y1970, y = y2000)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE)
```
...
Every one dollar increase in house value in 1970 translates to a five and a quarter dollar increase in 2000. On average house values in 2000 are 5.25 times what they were in 1970.

* Use the model to predict average housing values in 2000 for houses assessed values of $55,000, $60,000, and $65,000 in 1970
```{r}
predict(model, newdata = data.frame(y1970 = c(55000, 60000, 65000))) 
```

## Model Adequacy {-}

By definition the linear regression model is the "best fit" line through the data as defined by the smallest RSS. However the best fit line may not be adequate for our data. 

Model adequacy is about the quality of the inferences we can make with the model. It says nothing about the strength of the relationship between the response and explanatory variable. 

Checks on model adequacy alert us to where/how the model can be improved (e.g., transform the data, use a nonlinear term, etc).

When we infer something about nature with our linear regression model we are making four assumptions. All four assumptions imply that the residuals (observed values minus predicted values) should 'look' a certain way, or equivalently that the distribution of Y (response variable) conditional on X (explanatory variable) should look a certain way.
...
To the extent these assumptions are valid, the model is said to be _adequate_ in representing our data. A model can be statistically significant, but not adequate.

The first three assumptions are best examined with graphs. Note: We never prove an assumption is valid. We check to see if there is evidence to question its validity.
...
The variable is divided using the `cut()` function. Here we cut the `speed` into 5 intervals.
```{r}
...
We include these cuts as a ordered factor variable to the original `cars` data frame.
```{r}
library(dplyr)

cars <- cars %>%
  mutate(x_bins = cut(speed, breaks = 5))
```

We then map the `x_bins` variable to the x aesthetic to draw box plots.
```{r}
ggplot(data = cars, 
...
We effectively created sub-populations. The population of response values within a given interval of the explanatory variable. The number of intervals is the number of breaks specified by the `cut()` function.

Here we see evidence against the assumption of linearity. Further we see that the box size is increasing. That is the IQR range of breaking distance (depth of the box) is larger for faster moving cars. This creates doubt on the assumption of constant variance.

What about the assumption of normality? Although the normality assumption about the residuals is that the _conditional_ distribution of the residuals at each $x_i$ is adequately described by a normal distribution, in practice the residuals are examined collectively. The residuals are obtained by using the `resid()` function.
```{r}
res <- resid(model1)
res
```
...
We create a histogram and density of the model's residuals by typing
```{r}
...
We see that the histogram is not symmetric. There are more values to the right of the central set of values than to the left. The validity of the normality assumption is therefore under question.

Since departures from normality can occur simply because of sampling variation, the question arises as to whether that apparent skewness (asymmetry) we see in this set of residuals is significantly larger than expected by chance.
