...
A model with all the possible explanatory variables is called a 'full model'. The R squared value is largest when all explanatory variables are included in the model. But there is a cost. This cost is called 'model bias'. The model is biased toward the particular set of data you used to create the model.

If there is too much bias then the model will make poor predictions when given another set of data.
...
## The stepwise selection procedure {.unnumbered}
...
A strong word of caution about automated variable selection concerns final model stability or the robustness of the selected model to small perturbations of the data set. Bootstrap sampling with replacement or sub=sampling without replacement are needed to investigate and quantify model stability of selected models. The basic idea is to draw B samples from the original data set and to repeat the variable selection procedure on each of the samples.
...
* Use a data reduction method (principal components) to reduce the number of explanatory variables (p much less than n) 
...
## Transforming a response variable {.unnumbered}
...
A regression tree (decision tree) is a type of machine learning algorithm. When using a machine learning algorithm you are often focused on prediction rather than on making inference or understanding causality.
...
```{r}
predict(model.tr, 
        newdata = data.frame(Industry = 750, 
                             Population = 0, 
                             Wet.days = 0, 
                             Temp = 0, 
                             Wind = 0, 
                             Rain = 0))
predict(model.tr, 
        newdata = data.frame(Industry = 740, 
                             Population = 180, 
                             Wet.days = 0, 
                             Temp = 0, 
                             Wind = 0,
