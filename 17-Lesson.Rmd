...
Often you start with a model that includes many explanatory variables. This is because you are not sure what variables are important in statistically explaining the response variable.

A model with all explanatory variables is called a 'full model'. With linear regression, the R squared value is largest when all variables are included in the model. But there is a cost. This cost is called 'model bias'. The model is biased toward the particular set of data you used to create the model.
...
Let $y$ be the original response variable and $\lambda$ be the value of some power (e.g., 2 for squared and .5 for square root). Then a power tranformation of $y$ to $y_\lambda$ is given by
...
From the graph you can see that choosing a $\lambda$ between 0 and about -.6 will result in residuals that are normally distributed.
