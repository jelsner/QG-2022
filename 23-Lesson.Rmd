...
The graph is a network of nodes and links showing the contiguity pattern (topology). Each node is a tract and each link indicates spatial contiguity. The graph makes it clear that tracts in the center of the city have more neighbors than tracts away from the center.
...
With the weights matrix specified and saved as an object you are ready to compute a metric of spatial autocorrelation.
...
SL.sf <- read_sf(dsn = here::here("data", "scotlip"), 
                 layer = "scotlip")
plot(SL.sf$geometry)
```
