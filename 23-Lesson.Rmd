...
With observations made over space (spatial data), the assumption of independent values is usually not valid. This nearby locations tend to have similar values due to proximity.

Similarity due to proximity arises from:
...
Understanding the reason for spatial autocorrelation is important for causal inference because the causal mechanism  might be statistically confounded by it. Divorce rates are high in states in the South but so is the number of Waffle Houses.
...
Observations can be point locations or areal units.

## Spatial data as areal units {-}
...
Then you use the `read_sf()` function to get the data into your current session. You assign the spatial data frame the name `CC.sf`.
```{r}
CC.sf <- sf::read_sf(dsn = here::here("data", "columbus"), 
                     layer = "columbus")
```

The spatial data frame contains crime (residential burglaries and vehicle thefts per 1000 households) values in the column labeled `CRIME`. Among other variables it also include housing values and median household income in units of 1000 of dollars.

The object `CC.sf` behaves as a regular data frame but it is also `sf` (simple features) class that indicates there is a column that contains the geographic information.
```{r}
class(CC.sf)
```
...
The first twenty columns in the spatial data frame contain variables (attributes) and the last column contains the geometry information associated with each observation (row). The `geometry` column contains the spatial coordinates in well-known text (WKT) format.
...
Although you selected the variable `CRIME` the output also includes a summary on the geometry column.
...
You also have the option to make maps with functions from the {tmap} package.
```{r}
library(tmap)
...
Or with functions from the {ggplot2} package.
```{r, eval=FALSE}
library(ggplot2)
ggplot(data = CC.sf) + 
...
The maps show that high crime areas tend to cluster. Spatial autocorrelation quantifies this clustering. To compute the spatial autocorrelation we first need to define a list of neighbors for each tract (each tract has a polygon geometry).

## List of neighbors

We create a list of neighbors using the `poly2nb()` function from the {spdep} package.
```{r}
...
The function builds a list of neighbors for each polygon based on contiguity. Neighbors must share one or more boundary points. The default contiguity is defined as having at least one point in common. This is changed with the argument `queen = FALSE`.
...
You can graph the neighbors with the `plot()` method. The arguments include the neighbor list object (`nbs`) and the location of the polygon centers, which are extracted from the simple feature data frame using the `st_centroid()`.
```{r}
plot(nbs, 
...
The number of links per node (tract) [link number distribution] is obtained with the `summary()` method.
```{r}
summary(nbs)
``` 

We list the neighboring tracts for the first two tracts.
```{r}
nbs[[1]]
nbs[[2]]
```
...
Next we add weights to the neighbor list object. The weights specifying how 'close' each neighbor is. We do this  with the `nb2listw()` function. The function turns the neighbor list object into a spatial weights object. 
```{r}
wts <- nb2listw(nbs)
class(wts)
```
...
By default the `nb2listw()` function assumes each area has at least one neighbor. If this is not the case we need to specify how areas without neighbors are handled using the argument `zero.policy = TRUE`. This permits the weights list to be formed with zero-length weights vectors.
...
Now we repeat this calculation for every cell and then take the sum.
...
## Another example: North Carolina SIDS rates
...
Create a neighborhood list (`nb`) and a `listw` object (`wts`). First convert the simple feature to a spatial polygons data frame. Graph the neighborhood network.
```{r}
nbs <- poly2nb(SIDS.sf)
wts <- nb2listw(nbs)
plot(nbs, st_centroid(SIDS.sf)$geometry)
