...
Evidence in support of the null hypothesis that the reaction times are the same in the population is summarized by the $p$-value. The $p$-value is .009, so I conclude there is convincing evidence indicating cell phone use slows reaction times.
...
This test is used when you have two categorical variables from a single population and you want to determine whether there is a significant association between them.

For example, in an election survey, voters might be classified by drinker or non-drinker and voting preference (Democrat, Republican, or Independent). You use a chi-squared test for independence to determine whether drinking is related to voting preference.
