...
Predictions with the model are made without typing the coefficient values. Instead, first save the model as an object and then use the `predict()` method on the object. _Key_: In the `predict()` function you _must_ specify the data as a data frame using the `newdata =` argument.
