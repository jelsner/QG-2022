...
ggplot(data = heart.df, 
  ...
  geom_segment(mapping = aes(x = 50, xend = 50, 
                             y = predictedMean[2], 
                             yend = predictedMean[3]),
               color = "red")
...
Example: Predicting body mass from flipper length
...
To the extent these assumptions are valid, the model is said to be *adequate* in representing the data. *Key idea*: A model can be statistically significant, but not adequate.
...
-   These left overs (residuals) are interpreted as "controlled observations" (e.g. income controlling for percent graduates).
...
-   Regardless of how you decide to handle them, you need to let your readers know about these unusual cases.

(3) When interpreting a regression model fit to your data, you are making some implicit assumptions.
