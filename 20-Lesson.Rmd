...
Now all the AIC values _exceed_ the value given in the row labeled `<none>`, so we conclude there is no reason to remove any of the remaining variables so you are finished and this is your final model.
...
Both `drop1()` and `add1()` use the AIC as a criterion for choosing. To see it implemented we return to our full model and apply the `step()` function.
```{r}
step(modelFull)
```
...
You want to fit a multiple regression model that can be used to predict pollution levels (`Pollution`) from the explanatory variables including average temperature, the amount of industry, the population, the average wind speed, the amount of rain, and the number of wet days.
...
You choose the `lambda` that maximizes the likelihood. In this case it is about -.25. Note: $\lambda$ is a random variable and thus the 95% uncertainty interval provides a range of values. Here the log-likelihood is maximized between -.75 and .2.
...
Regression trees are a popular type of machine learning algorithm. Machine learning algorithms focus on prediction. Given a set of data (usually with many observations (cases) and variables) can we accurately predict what the outcome (response) will be when we give the algorithm a new observation.

Linear regression is a model for the conditional mean. The average of the response variable for given values of the explanatory variables. A regression tree is another model for the conditional mean, but where the _conditioning_ is done by partitioning the space defined by the explanatory variables.

To create a regression tree use the `tree()` function from the {tree} package. Here create a regression tree for the response variable `Pollution` from the data frame `pollute.df`.
```{r}
library(tree)
...
There is no table of coefficients. Instead we get a table listing the explanatory variables, splits, number of cases, and deviances (RSS).

A plot helps us make sense of this output. Here we use the generic `plot()` method and add text with the `text()` function.
```{r}
plot(tree.model)
text(tree.model)
```

The plot shows an upside down tree. The branches (called splits) are the rules. Rules (involve the 'less than' symbol) are applied to the explanatory variables. Branches end with leaves. The value at the leaf is the mean of the response variable for that subset of observations. 
...
The variable explaining the greatest amount of variation in the response variable is selected first. Since the first split is on the variable `Industry` at a value of 748, let's look at how `Pollution` is partitioned on this variable. First create a logical vector called `split` containing `TRUE` and `FALSE` depending on whether the value is less than 748. Then use this variable to select the values of `Pollution` (from the `pollute.df`) to compute the mean.
```{r}
...
Or using the grammar of data.
```{r}
library(dplyr)
pollute.df %>%
  mutate(split = Industry < 748) %>%
  group_by(split) %>%
  summarize(Avg = mean(Pollution))
```

Thus when `Industry` is less than 748 the mean value for `Pollution` is 24.92 and when it is greater or equal to 748 the mean value for `Pollution` is 67 and when it is less than 748, the mean value of `Pollution` is 24.92. Graphically we can illustrate this using the following lines of code (code chunk).
```{r}
library(ggplot2)

ggplot(pollute.df, aes(x = Industry, y = Pollution)) +
  geom_point(size = 2) +
  geom_vline(xintercept = 748, linetype = 'dashed') +
  geom_segment(x = 748, xend = max(pollute.df$Industry), y = 67, yend = 67) +
  geom_segment(x = 0, xend = 748, y = 24.92, yend = 24.92)
```
...
The relatively large $t$ value (greater than 3) results in a small $p$-value (less than .05) indicating we can safely reject the null hypothesis.

Let's try another split. What do we find?
```{r}
...
The $t$ value goes down and the $p$-value increases indicating that splitting at 720 is not as good as splitting at 748.

Can we find a better split of `Pollution` along `Industry`?  Here is how we can answer that question.
```{r}
tvalue <- numeric()
...
ggplot(df, aes(x = Industry, y = tvalue)) +
  geom_line() +
  geom_vline(xintercept = 748, color = "red") +
  xlab("Industry") + ylab("t statistic")
```

The split value (748) found by the regression tree for the variable `Industry` is maximal over all such splits.

How it works. For a given explanatory variable:

* Select a candidate split value.
* Compute the mean of the response above and below the value.
* Compute the deviance (residual sum of squares) from the two means.
* Go through all possible splits.
* Look to see which split gives the lowest deviance. This is the split that gives the largest $t$ statistic.
* Split the entire data set into high and low subsets based on this split value.
* Repeat on each subset of the data on either side of the first split.
* Keep going until no additional reduction in deviance is obtained, or there are too few data values to merit further divisions.
...
The two key questions are: (1) Which variables to use in the split, and (2) how to best achieve the split given a variable. Tree regression is analogous to forward selection in stepwise regression. Add in the most important variable first then see if additional variables decrease the deviance keeping track of the cost associated with the loss of the degree of freedom.

Model output is printed by typing the name of the model object.
```{r, eval=FALSE}
tree.model
```
...
This kind of nuanced and contingent explanation is made with a regression tree. The cost is the potential to interpret noise as signal.
...
Note that we need to specify all the explanatory variables even if they are not involved with the final predicted value.
...
Prediction amounts to following the rules defined by the regression tree. We start with the value of the variable `Industry` and work our way down until we hit a terminal node. The value of the response variable representing the subset mean is the predicted value.

The standard error on the predicted value is more difficult to work out. We could use the standard error on the subset mean for that leaf. But this error ignores the uncertainty surrounding the other decisions (branches) that came before.

A tree regression is a machine learning algorithm. Machine learning algorithms are used for prediction, but not typically for inference. There is no simple way to get an uncertainty estimate about the predicted level of pollution.
...
Here we see the largest tree (6 terminal nodes) results in the smallest deviance (8877). By pruning one branch the deviance increases (9240) but only by a relatively small amount (364 or 4%). Similarly by pruning a second branch the deviance increases, this time a bit more. It is helpful to plot the deviance as a function of tree size.
```{r}
plot(prune.tree(tree.model))
```

Here the plot is opposite of the table. Deviance declines with increasing complexity. Code challenge: Make this using the grammar of graphs. Hint: save the prune tree object and then create a data frame from the object lists. Then use `scale_x_reverse()`.

We can see that the amount of decline diminishes with each node (diminishing return). In fact, we could argue that a tree with 4 terminal nodes is sufficiently complex as the drop in deviance from 4 to 5 nodes is relatively small.
