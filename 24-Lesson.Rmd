...
- Estimating statistical significance of spatial autocorrelation
- Spatial autocorrelation in model residuals

## Various spatial weighting schemes {-}

Let's consider another data set. SIDS rates in the 100 counties of North Carolina for the period July 1, 1979 to June 30, 1984. The data are on my website as zipped shapefiles. Download and unzip the data in your working directory.
```{r}
...
Import the data with the `read_sf()` function from the {sf} package. Set the coordinate reference system to WGS 1984 with latitude/longitude coordinates using the ESPG code of 4326.
```{r}
SIDS.sf <- sf::read_sf(dsn = "sids2")
...
There appears to be some clustering of the SIDS rates. Counties with high rates tend to be surrounded by counties with high rates and counties with low rates tend to be surrounded by counties with low rates.
...
We noted last week that Moran I is the slope of a regression of the spatial lag variable onto the variable itself.

The spatial lag variable consists of the average of the neighboring values. To compute the spatial lag variable we use the `lag.listw()` function.
...
Even if the county SIDS rates are randomly distributed across the state, the value of Moran I will not be zero. If we want to use I to establish statistically significant clustering, we need a way to guard against this randomness.

Said another way: Is the value of Moran I significant with respect to the null hypothesis of no spatial autocorrelation? 

One way to answer this question is to draw an uncertainty band on the regression line and see if a horizontal line can be placed within the band. If not, then I is statistically different than what we would expect if the null hypothesis of no spatial autocorrelation were true.
...
Thus we reject the null hypothesis and conclude there is weak but significant spatial autocorrelation in SIDS rates across North Carolina at the county level.
...
A spatial regression model may be needed whenever the residuals resulting from a aspatial regression model exhibit significant spatial autocorrelation. 

A common way to proceed is to first regress the response variable onto the explanatory variables and check for spatial autocorrelation in the residuals.
...
The next step is to choose a spatial regression model. Before we go there, let's look at one more example.
...
Again, we first need to assign the neighborhoods and the associated weights between the each region and each neighbor of that region.
```{r}
nbs <- poly2nb(PE.sf)
wts <- nb2listw(nbs)
```

Another way to do this is to specify the number of neighbors and define who the neighbors are based on distance. We do this with the `knearneigh()` function. We first extract the coordinates of the polygon centroids.
```{r}
library(dplyr)
...
The output is a list of five elements with the first being a matrix of dimension number of counties by the number of neighbors. We see here that the neighborhoods are non-symmetric. County 3 is a neighbor of county 2, but county 2 is not a neighbor of county 3. This is important since some spatial regression models require symmetric neighborhood definitions.
...
library(maps)
counties.sf <- st_as_sf(map("county", "florida", plot = FALSE, fill = TRUE))

covid.sf <- counties.sf %>%
  mutate(population = pop$population,
         deaths = deaths$`7/1/20`,
         DeathRate = deaths/population * 100000)

tm_shape(covid.sf) +
  tm_fill("DeathRate", title = "") +
  tm_borders(col = "gray70") +
  tm_layout(title = "Death rate as of Nov 20, 2020 [per 100000]",
            legend.outside = TRUE)

nbs <- poly2nb(covid.sf)
wts <- nb2listw(nbs)
m <- length(covid.sf$DeathRate)
s <- Szero(wts)

moran.test(covid.sf$DeathRate, 
      listw = wts)
```