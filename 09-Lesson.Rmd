...
-   Mapping using functions from {tmap}
...
The print out includes information about the file. The object shows up in your environment as a data frame with 56 observations and 10 variables. Each feature is either a state or territory.
...
```{r}
ggplot(data = USA_48.sf) +
  geom_sf(mapping = aes(fill = AWATER/ALAND * 100))
```
...
ggplot() + 
  geom_sf(data = USA_48.sf) + 
  geom_sf(data = airports.sf, 
          mapping = aes(size = alt), 
...
## Changing projections {.unnumbered}
...
```{r}
( map_nz1 <- map_nz +
  tmap::tm_shape(shp = spDataLarge::nz_elev) + 
...
```{r}
( map_nz2 <- map_nz1 +
  tmap::tm_shape(shp = nz_water.sfc) + 
    tmap::tm_lines() )
```
...
```{r}
( map_nz3 <- map_nz2 +
  tmap::tm_shape(shp = spData::nz_height) + 
    tmap::tm_dots() )
```
...
Small multiples are also generated by assigning more than one value to an aesthetic arguments.
...
## Making an interactive map {.unnumbered}
