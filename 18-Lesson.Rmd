...
Linear regression provides an adequate model for our data under the following four assumptions.
...
When the data have a spatial component it's a good idea to make a map. Here we create a choropleth map using functions from the {tmap} package (`09-Lesson.Rmd`).
```{r}
library(tmap)
...
We can say that the effect is quite precise.
...
A plot of the model residuals as a function of the explanatory variables lets us easily identify which states have the largest positive and negative residuals. To make the plot more readable we use the `geom_label_repel()` function from the {ggrepel} package. This labels the points while avoiding overlapping labels.
```{r}
library(ggrepel)
...
* We often want to do more than just summarize the relationship between variables. That is go beyond reporting the correlation.
...
* If there are we should re-fit the regression model without those observations and compare results.
* Regardless of how we decide to handle them, we need to let our readers know about these unusual cases.

(3) When interpreting a regression model fit to our data, we are making some implicit assumptions.

* Before accepting a model we need to examine those assumptions to make sure they are tenable.
* The model fit may be excellent, but we can't be sure our conclusions are correct unless we can defend the assumptions.
* Examining the model residuals helps us defend (if warranted) the assumptions.
...
Let's return to the Florida precipitation data we looked at earlier in the semester. Import the data and make a line plot showing March precipitation values each year. Draw the best-fit line through these values.
```{r}
...
There is an upward trend (increasing precipitation) but we can see that the uncertainty ribbon would allow a horizontal line. Thus we do not anticipate a statistically significant trend term in a regression model.

To verify we regress March precipitation onto year.
```{r}
model <- lm(Mar ~ Year, 
            data = FLp)
summary(model)
...
We see that the statewide annual average precipitation has been increasing by .01 inches per year. This upward trend has a $p$-value of .04 providing suggestive but inconclusive evidence against the null hypothesis of no trend. The larger the $p$-value the more evidence we have that the trend is not statistically significant.

We can fit regression models for all months. First convert the wide to a long data frame.
```{r}
library(tidyr)
FLpL <- FLp %>%
...
Then to fit trend models _separately_ for each month we use the `do()` function from the {dplyr} package together with the `tidy()` function from the {broom} package.
```{r}
library(broom)

FLpL %>% 
  mutate(MonthF = factor(Month, levels = month.abb)) %>%
  group_by(MonthF) %>%
  do(tidy(lm(Precipitation ~ Year, data = .)))
```
...
Simpson's paradox occurs when a trend appears in several different groups of data but disappears or reverses when these groups are combined. It is often encountered in social-science and medical-science statistics and is particularly problematic when data are aggregated at various scales (spatial or otherwise). 

The paradox can be resolved when causal relations are appropriately addressed in the statistical modeling.

As an example, consider again the relationship between bill length vs bill depth in the Palmer penguins without regards to species.
```{r}
...
However the relationship is across all species of penguins. If we group by species we see that for each species the relationship is the opposite.
```{r}
p + 
...
That is bill depth _increases_ with increasing bill length. So we conclude that the negative relationship is an artifact of lumping the penguins with significantly different body masses together.