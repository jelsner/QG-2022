...
Consider again the Florida rainfall data. Import the data and apply the not available label (`NA`) to any value that is recorded as -9.900.
...
To see this you can generate values from various standard distributions and then make a histogram. These standard distributions are used in statistical analysis as reference. This means that they allow researchers to easily compare data and groups of samples.
...
Here you generate values with the same mean and standard deviation as the rainfall values. Since there are 118 March rainfall values (one for each year) you set `n = 118`.
...
When you knit your Rmd file and render it to HTML any plots that are generated from code get output as a png file in your project directory.
