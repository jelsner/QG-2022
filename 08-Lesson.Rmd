...
Last week we focused on manipulating data. This is important for getting to the next stage, which is data visualization. Being able to visualize data in an efficient way allows us to quickly gain valuable insights that we can't get just by looking at raw values. 

We use the {ggplot2} package because it provides functions to efficiently produce plots. The package was developed from a theory known as the grammar of graphics (Leland Wilkinson).

At a basic level, graphics/plots/charts (we use these terms interchangeably) provide a nice way to explore the patterns in data; the presence of extreme values, distributions of individual variables, and relationships between groups of variables. 

Graphics should emphasize the findings and insights we want our audience to understand. This requires a balance. On the one hand, we want to highlight as many interesting findings as possible. On the other hand, we don’t want to include so much information that it overwhelms the audience.

Plots help us identify patterns and unusual values in our data. We’ll see that a common extension of these ideas is to compare the distribution of one numerical variable, such as what are the center and spread of the values, as we go across the levels of a different categorical variable.

The grammar of graphics specifies how a plot translates data to attributes and geometric objects. Attributes are things like location on along an axes, color, shape, and size. Geometric objects are things like points, lines, bars, and polygons. The type of plot depends on the geometric object, which is specified as a function. 

Function names for geometric objects in {ggplot2} begin with `geom_`. For example, to create a scatter plot of points the `geom_point()` function is used. The plot may contain manipulations of the data (like `geom_histogram()`). Similar plots on subsets of the data by using the `facet_` functions (similar to the `group_by()` function).
...
A simple graph is the bar chart. It shows the number of cases within each group. Consider again the annual hurricane counts.
...
We begin a plot with the function `ggplot()` that creates a coordinate system that we add layers to. The first argument of `ggplot()` is the data frame to use in the graph. So `ggplot(data = LH.df)` creates an empty graph.

We complete our graph by adding one or more layers. The function `geom_bar()` adds a layer of bars to our plot, which creates a bar chart. 
...
For example to change the bar color use the `fill =` argument in the function `geom_bar()`. To change the orientation of the bars use the layer function `coord_flip`, and to change the background use the layer function `theme_minimal()`. We make changes to the look of the plot with additional layers.
```{r}
ggplot(data = piePref.df) + 
...
In the above example we manually reordered the levels in the factor vector `pie` according to preference. Let's see how to do this automatically.

Consider storm intensity of tropical cyclones during 2017. First we create two vectors one numeric containing the minimum pressures (millibars) and the other character containing the storm names.
```{r}
...
Using the `mutate()` function we learned last week we can add a column to a data frame where the column is an ordered factor. We note that it is the difference in pressure (`deltaP` for short) between the air outside the tropical cyclone and the air in the center that causes the winds. Cyclones with a large pressure difference are stronger in terms of their wind speed.

Typically the air outside is about 1014 mb so we compute `deltaP` and then reorder the tropical cyclone names using this computed variable.
```{r}
library(dplyr)

df <- data.frame(name, minP) %>%
  mutate(deltaP = 1014 - minP,
         nameOrderedFactor = reorder(name, deltaP))
```

Finally we plot the bar chart. Since there is no tabulation of the values we use `geom_col()` instead of `geom_bar()`.
```{r}
ggplot(data = df) + 
...
df <- read.csv(file = "TLH_SOD1892.csv",
  ...
  mutate(Year = year(DATE),
         Month = month(DATE)) %>%
  filter(Year >= 1980 & Month == 9) %>%
  group_by(Year) %>%
  summarize(TotalPrecip = sum(PRCP)) %>%
  mutate(Year = reorder(as.factor(Year), TotalPrecip))
...
A density plot is a smoothed histogram with units of probability on the vertical axis. It's motivated by the fact that for a continuous variable, the probability that the variable takes on any particular value is 0. Instead we need a range of values over which we can define a probability.
...
Note that the column `Month` is a character vector. When we make a plot using this variable the order will be alphabetical. So instead we change it to a factor vector with levels equal to the month abbreviations.
```{r}
month.abb
...
The `pivot_longer()` function takes all the columns to pivot into a longer format. Here we chose them all EXCEPT the one named after the `-` sign (`Year`). All variables are measured (precipitation in units of inches) except `Year`.

The resulting long data frame has the `Year` variable in the first column and the remaining column names as the `name` variable in the second column. We change the default `name` to `Month` by specifying the `names_to = "Month"` argument. The third column contains the corresponding precipitation values all in a single column names `value`. We change the default `value` by specifying the `values_to = "Precipitation"`.

Note that we can reverse this with the `pivot_wider()` function.
```{r}
FLpW.df <- FLpL.df %>%
...
```{r}
ggplot(FLpL.df, aes(x = Month, y = Precipitation)) + 
  geom_boxplot() +
  ylab("Precipitation (in)") +
