...
The mean and variance are common summary statistics calculated on data. Typically there are more data values close to the mean than far from it. A variable that can be described with a normal distribution will have a random value that lies within two standard deviations of its mean about 95% of the time.

The median is a statistic defined as the middle value in the set of values. Half the values are above the median and half the values are below the median.

For example, consider a set of seven data values. Here the seven values are generated randomly from a standard normal distribution. A standard normal distribution has a mean of zero and a variance of one.

First you use the `set.seed()` function with a particular seed value to guarantee everyone (with that seed number) will get the same set of values. The argument `n = 7` in the random number generator function `rnorm()` specifies the number of values.
...
y
```
Next use the `sort()` function on the vector of values to arrange them from lowest (on the left) to highest. 
...
With an odd number of values, the median is the middle value. With an even number of values, the median is the average of the two middle values.
...
Consider the wealth of five bar patrons (in 1000s of \$).

```{r}
patrons <- c(50, 60, 100, 75, 200)
```

Now consider the same bar and patrons after a billionaire walks in.

```{r}
patrons_with_mm <- c(patrons, 1000000)
```
...
The difference in the mean wealth with and without the billionaire present is substantial while the difference in median wealth with and without the billionaire is small.

Statistics that are not influenced be a few values far from the bulk of the data are called *resistant*.

The `cfb` data set from the {UsingR} package has a sample of data from the survey of individual consumer finances conducted by the U.S. Federal Reserve Board (in 2001).

Show the first ten rows and ten columns of the data frame by typing the name of the data object (`UsingR::cfb`).

```{r}
UsingR::cfb
```

Each row is an individual with columns indicating savings, mutual funds, stocks, and other assets. Some of the income values are much higher than the bulk of the data. This tendency is common in income distributions. A few people tend to accumulate enormous wealth.

Next, assign to an object called `income` the column `INCOME` in the data frame, and determine the mean and median incomes. The income is the individual annual income in 2001 for each participant of the survey.

```{r}
income <- UsingR::cfb$INCOME
mean(income)
median(income)
```
...
All the elements of a vector must have the same type. That is you can't mix numeric values with character strings.
...
If you mix element types within a data vector, all elements will change into the 'lowest' common type, which is usually a character. Arithmetic does not work on vectors that contain character strings.
...
The vector object is now an ordered factor instead of a character. Printing the object results in a list of the elements in the vector and a list of the levels. Note: if you do the same for the `wx` object, the order is alphabetical by default. Try it.
