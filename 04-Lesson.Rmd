...
The mean and variance are common summary statistics calculated on data. There are usually more data values close to the mean than far from it. A variable that can be described with a normal distribution will have most values (~95% of them) within two standard deviations of the mean.

The median is a statistic defined as the middle value in a set of values. Half the values are above the median and half the values are below the median.
...
First use the `set.seed()` function with a particular seed value to guarantee you (with that seed number) will get the same set of values. The argument `n = 7` in the random number generator function `rnorm()` specifies the number of values.
